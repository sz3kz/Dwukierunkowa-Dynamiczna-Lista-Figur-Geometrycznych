	...
	while (1){
		...
		switch(option){
			case 'a':
				...
				EMPTY_STDIN;
				if (current == NULL) 
					current = create_node(shape);
				else 
					append_node(current, create_node(shape));
				break;
			case 'p':
				...
				if (current == NULL) 
					...
					prepend_node(current, create_node(shape));
				break;
			case 'd':
			...
			case 'n':
				...
				if (current->next != NULL) 
					...
					puts("There exists no next node.");
				break;
			case 'b':
				...
				if (current->prev != NULL) 
					...
					puts("There exists no node before this one.");
				break;
			case 'x':
